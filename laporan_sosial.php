...
$queryMasuk = "SELECT SUM(masuk) AS total_masuk FROM kas_sosial WHERE jenis = 'Masuk'";
$resultMasuk = mysqli_query($conn, $queryMasuk);
$totalMasuk = mysqli_fetch_assoc($resultMasuk)['total_masuk'];

$queryKeluar = "SELECT SUM(keluar) AS total_keluar FROM kas_sosial WHERE jenis = 'Keluar'";
